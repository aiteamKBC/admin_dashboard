...
export default function AnalyticsMeetings({ onOpenSidebar }: { onOpenSidebar?: () => void }) {
  ...
  // Parse evidence data from string/object payloads.
  const parseEvidenceData = (input: any) => {
    ...
    return parsed.map((item: any) => ({
      ...
      componentName: String(item?.ComponentName ?? item?.componentName ?? ""),
      name: String(item?.Name ?? item?.name ?? "Unnamed Evidence"),
  ...
  const handleMarkEvidence = async (evidenceItem: any) => {
    ...
    // Show waiting popup immediately when user clicks Mark.
    setResultModal({
      open: true,
      data: null,
      loading: true,
    ...
    try {
      ...
      // Open result modal. If report is not ready yet, start polling.
      const resolvedEvidenceId = String(result?.data?.evidence_id || modalEvidenceId || "");
      const resolvedProgram = String(result?.data?.program || modalProgram || "");
      const hasReport = !!result?.data?.marking_report;

      setResultModal({
        open: true,
        data: hasReport ? result.data : null,
        loading: !hasReport,
        error: null,
  ...
  useEffect(() => {
    if (!resultModal.open || !resultModal.loading || !resultModal.evidenceId || !resultModal.program) {
      return;
    }
    ...
    const pollOnce = async () => {
      try {
        ...
        const params = new URLSearchParams({
          evidence_id: String(resultModal.evidenceId),
          program: String(resultModal.program),
        });

        const response = await fetch(`/api/accounts/poll-marking-report/?${params}`, {
          headers: {
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json",
          },
        ...
        if (payload?.found && payload?.data?.marking_report) {
          setResultModal((prev) => ({
            ...prev,
            loading: false,
            error: null,
        ...
        setResultModal((prev) => {
          const nextCount = prev.pollCount + 1;
          if (nextCount >= prev.maxPolls) {
            return {
              ...prev,
              loading: false,
              error: "Still waiting for report after 10 minutes. Please try again shortly.",
              pollCount: nextCount,
            };
          }
          return {
        ...
        setResultModal((prev) => ({
          ...prev,
          loading: false,
          error: err?.message || "Failed while waiting for marking report",
        }));
      }
    };
  ...
  }, [resultModal.open, resultModal.loading, resultModal.evidenceId, resultModal.program]);
  ...
  const evidenceItems = useMemo(
    () => parseEvidenceData(evidenceModal.data?.evidence),
    [evidenceModal.data?.evidence]
  );

  const activeProgram = String(evidenceModal.data?.program || evidenceModal.data?.group || "").trim();
      ...
      {evidenceModal.open && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 backdrop-blur-sm px-3">
          <div className="bg-white w-full max-w-5xl h-[80vh] rounded-2xl shadow-xl flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <div>
                <h3 className="text-lg font-semibold text-[#442F73]">Evidence Data</h3>
                <p className="text-xs text-gray-500">
                  {evidenceModal.studentName || evidenceModal.studentEmail || evidenceModal.studentId || "Student"}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="px-5 py-3 border-b bg-[#f7f7fc]">
              <div className="inline-flex items-center rounded-md border border-blue-200 bg-blue-100 px-4 py-2 text-sm font-semibold text-blue-900">
                Program: {activeProgram || "Unknown"}
              </div>
            </div>

            <div className="flex-1 overflow-y-auto p-5 custom-scroll custom-scroll--soft">
              {evidenceModal.loading ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-600">
                  <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#7743DB] mb-3" />
                  Loading evidence...
                </div>
              ) : evidenceModal.error ? (
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 text-sm text-gray-600">
                  No evidence found for this student in the selected program tab.
                </div>
              ) : (
                <div className="space-y-3">
                  {evidenceItems.map((item: any) => {
                    const inProgress = String(evidenceModal.markingInProgress || "") === String(item.id);
                    return (
                      <div key={item.id || `${item.name}-${item.createdDate}`} className="rounded-xl border border-gray-200 bg-white p-4">
                        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
                          <div className="min-w-0">
                            <div className="text-sm font-semibold text-[#241453] break-words">{item.name || "Unnamed Evidence"}</div>
                            <div className="mt-1 text-xs text-gray-600 break-words">
                              <span className="font-medium">Evidence ID:</span> {item.id || "—"}
                              <span className="mx-2 text-gray-300">•</span>
                              <span className="font-medium">Status:</span> {item.status || "—"}
                            </div>
                            <div className="mt-1 text-xs text-gray-600 break-words">
                              <span className="font-medium">Component ID:</span> {item.componentId || "—"}
                              {item.componentName ? (
                                <>
                                  <span className="mx-2 text-gray-300">•</span>
                                  <span className="font-medium">Component:</span> {item.componentName}
                                </>
                              ) : null}
                            </div>
                            {item.url ? (
                              <a
                                ...
                                className="mt-2 inline-flex text-xs text-blue-700 hover:text-blue-900 underline"
                              >
                                Open evidence
                              </a>
                            ) : null}
                          ...
                          <button
                            ...
                            disabled={inProgress}
                            className={[
                              "text-xs px-4 py-2 rounded-lg border transition h-fit",
                              inProgress
                                ? "border-gray-300 text-gray-400 bg-gray-100 cursor-not-allowed"
                                : "border-blue-300 text-blue-700 bg-white hover:bg-blue-600 hover:text-white",
                            ].join(" ")}
                          >
                            {inProgress ? "Marking..." : "Mark evidence"}
                          </button>
                        </div>
                      </div>
                    );
            ...
            <div className="p-5 overflow-y-auto space-y-3">
              {resultModal.loading ? (
                <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
                  <div className="font-semibold">Generating marking report...</div>
                  <div className="mt-1">This usually takes up to 5 minutes.</div>
                  <div className="mt-2 text-xs text-blue-700">
                    Checks: {resultModal.pollCount} / {resultModal.maxPolls}
                  </div>
                </div>
              ) : resultModal.data?.marking_report ? (
                <>
                  <div className="text-xs text-gray-600">
                    <span className="font-medium">Evidence ID:</span> {resultModal.data?.evidence_id || "—"}
                    <span className="mx-2 text-gray-300">•</span>
                    <span className="font-medium">Program:</span> {resultModal.data?.program || "—"}
                  </div>
                  <div className="rounded-xl border border-gray-200 bg-[#fafafe] p-4">
                ...
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                  Marking result has not been generated yet.
                </div>
              )}
            </div>
          </div>
